...
class Metasploit3 < Msf::Auxiliary
  ...
  def initialize
    ...
    [
      Opt::RPORT(137),
      OptBool.new('STORE_LOOT', [false, 'Store file in loot.', false]),
      OptBool.new('EXTRA_INFO', [false, 'Return all information from NetBIOS Name Table.', false])
    ], self.class)
  end
  ...
  def scanner_postscan(batch)
    ...
    @results.keys.each do |ip|
      ...
      if datastore['EXTRA_INFO']
        print_status("#{ip} [#{host[:name]}] OS:#{os}#{user}")
     
        # Results table holds data
        results_table = Rex::Ui::Text::Table.new(
            'Header'     => "NetBIOS Name Table",
            'Indent'     => 1,
            'SortIndex'  => -1,
            'Columns'    => ['Name', 'Code', 'Service', 'Type']
        )
      
        # Populate results table
        host[:info].each do |info|
            results_table << info
        end
      
        print_line results_table.to_s
        print_line "MAC: #{host[:mac]}"
        
        if datastore['STORE_LOOT']
            stored_path = store_loot('netbios.info', 'text/plain', ip, results_table.to_csv)
  ...
  def scanner_process(data, shost, sport)
    ...
    when 0x21
      ...
      inf = ''
      # Array to keep member's names and info
      info = []
      names.each do |name|
        inf << name[0]
        nbcode = "<%.2x>" % name[1]
        ...
        when "<03>"
          nbdesc = "Messenger"
        when "<1c>"
          nbdesc = "Domain Controller"
        when "<1b>"
          nbdesc = "Domain Master Browser"
        when "<1e>"
          nbdesc = "Browser Election Service"
        when "<1d>"
          nbdesc = "Local Master Browser"
        when "<01>"
          nbdesc = "Browser"
        else
          nbdesc = "-"
        ...
        info << [name[0], nbcode, nbdesc, nbtype]
      end
      inf << " : #{maddr}"
      
      @results[shost][:info] = info
